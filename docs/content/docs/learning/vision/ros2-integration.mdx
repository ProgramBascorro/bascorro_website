...
**Steps**:
1. Subscribe image dan camera info.
2. Convert ke OpenCV via `cv_bridge`.
3. Jalankan deteksi dan publish hasil.
**Pitfalls**: encoding salah membuat image terdistorsi.
**Validation**: debug image tampil benar di `rqt_image_view`.
...
2. Pass parameter ke node.
3. Gunakan remap untuk topic downstream.
**Pitfalls**: parameter string vs numeric yang salah tipe.
**Validation**: `ros2 param get` menampilkan nilai sesuai launch.
