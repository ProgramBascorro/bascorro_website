    ...
    <div className="min-h-screen bg-[#f3f4f6]">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          >
            ...
            <h1 className="font-display font-black text-4xl sm:text-5xl md:text-6xl lg:text-7xl leading-tight tracking-tighter mb-6">
              RoboCup
              <span className="text-accent-yellow"> &amp; KRI</span>
              <br />
              <span className="text-white/90">Humanoid Competitions</span>
            </h1>
      ...
      <section className="px-6 py-16 md:py-24 bg-white">
        <div className="max-w-7xl mx-auto">
          <SectionHeader title="What is RoboCup?" subtitle="International" />
      ...
      <section className="px-6 py-16 md:py-24 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <SectionHeader
            title={KRI_OVERVIEW.title}
            subtitle={KRI_OVERVIEW.subtitle}
          />
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div>
              <p className="text-gray-600 leading-relaxed mb-6">
                {KRI_OVERVIEW.description}
              </p>
              <p className="text-gray-600 leading-relaxed">
                KRSBI-Humanoid mirrors the spirit of RoboCup at the national
                level, testing perception, locomotion, and strategy under
                strict rules and real match conditions.
              </p>
            </div>
      ...
      <section className="px-6 py-16 md:py-24 bg-white">
        <div className="max-w-7xl mx-auto">
          <SectionHeader title="KRI Focus Areas" subtitle="KRSBI-Humanoid" />
          <p className="text-gray-600 max-w-2xl mb-12">
            KRSBI-Humanoid prioritizes full-stack performance, from hardware
            readiness to match strategy. These are the areas we focus on for the
            2026 target.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {KRI_FOCUS_AREAS.map((area, i) => {
              const Icon = IconMap[area.icon] || Activity;
              return (
                <motion.div
                  key={i}
                  className="border border-gray-200 p-6 rounded-2xl hover:border-undip-blue/40 hover:shadow-lg transition-all"
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true, margin: '-50px' }}
                  transition={{ delay: i * 0.1, duration: 0.5 }}
                >
                  <div className="w-12 h-12 bg-undip-blue/10 rounded-xl flex items-center justify-center text-undip-blue mb-6">
                    <Icon size={24} />
                  </div>
                  <h4 className="font-bold text-lg mb-2">{area.title}</h4>
      ...
      <section className="px-6 py-16 md:py-24 bg-[#f3f4f6]">
        <div className="max-w-7xl mx-auto">
          <SectionHeader title="RoboCup Events" subtitle="Competition Calendar" />
      ...
      <section className="px-6 py-16 md:py-24 bg-white">
        <div className="max-w-7xl mx-auto">
          <SectionHeader title="KRI Events" subtitle="KRSBI-Humanoid" />
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {KRI_EVENTS.map((event, i) => (
              <motion.div
                key={i}
                className={`bg-gray-50 rounded-2xl p-6 border ${event.status === 'upcoming' ? 'border-undip-blue' : 'border-gray-200'} ${event.status === 'upcoming' ? 'ring-2 ring-undip-blue/20' : ''}`}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, margin: '-50px' }}
                transition={{ delay: i * 0.05, duration: 0.4 }}
              >
                ...
                <h3 className="font-bold text-xl mb-1">KRI {event.year}</h3>
                <p className="text-gray-600">{event.location}</p>
              </motion.div>
            ))}
          </div>
            ...
            <div>
              <div className="flex items-center gap-3 mb-6">
                ...
                <p className="text-white/70 leading-relaxed mb-6">
                  ...
                  working toward competing in both RoboCup Humanoid League and
                  KRSBI-Humanoid. Our focus is on the {BASCORRO_JOURNEY.currentFocus},
                  building expertise in locomotion, computer vision, and autonomous
                  decision-making.
                </p>
