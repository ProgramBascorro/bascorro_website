    ...
    present_pitch_(0)
{
  ...
  nh_ = rclcpp::Node::make_shared("soccer_demo");

  std::string default_path = ament_index_cpp::get_package_share_directory("op3_gui_demo") + "/config/gui_config.yaml";
  std::string path = nh_->declare_parameter<std::string>("demo_config", default_path);
  ...
  is_grass_ = nh_->declare_parameter<bool>("grass_demo", false);
}
...
void SoccerDemo::callbackThread()
{
  ...
  module_control_pub_ = nh_->create_publisher<robotis_controller_msgs::msg::JointCtrlModule>("/robotis/set_joint_ctrl_modules", 10);
  motion_index_pub_ = nh_->create_publisher<std_msgs::msg::Int32>("/robotis/action/page_num", 10);
  rgb_led_pub_ = nh_->create_publisher<robotis_controller_msgs::msg::SyncWriteItem>("/robotis/sync_write_item", 10);

  button_sub_ = nh_->create_subscription<std_msgs::msg::String>("/robotis/open_cr/button", 10, std::bind(&SoccerDemo::buttonHandlerCallback, this, std::placeholders::_1));
  demo_command_sub_ = nh_->create_subscription<std_msgs::msg::String>("/robotis/demo_command", 10, std::bind(&SoccerDemo::demoCommandCallback, this, std::placeholders::_1));
  imu_data_sub_ = nh_->create_subscription<sensor_msgs::msg::Imu>("/robotis/open_cr/imu", 10, std::bind(&SoccerDemo::imuDataCallback, this, std::placeholders::_1));

  is_running_client_ = nh_->create_client<op3_action_module_msgs::srv::IsRunning>("/robotis/action/is_running");
  set_joint_module_client_ = nh_->create_client<robotis_controller_msgs::srv::SetJointModule>("/robotis/set_present_joint_ctrl_modules");

  test_pub_ = nh_->create_publisher<std_msgs::msg::String>("/debug_text", 10);
...
void SoccerDemo::callServiceSettingModule(const robotis_controller_msgs::msg::JointCtrlModule &modules)
{
  ...
  {
    RCLCPP_ERROR(nh_->get_logger(), "Failed to set module Soccer");
    return;
  }
}
...
void SoccerDemo::parseJointNameFromYaml(const std::string &path)
{
  ...
  {
    RCLCPP_ERROR(nh_->get_logger(), "Fail to load id_joint table yaml.");
    return;
  }
...
void SoccerDemo::imuDataCallback(const sensor_msgs::msg::Imu::SharedPtr msg)
{
  ...
  RCLCPP_INFO_COND(DEBUG_PRINT, nh_->get_logger(), "Roll : %3.2f, Pitch : %2.2f", rpy_orientation.coeff(0, 0), rpy_orientation.coeff(1, 0));
...
void SoccerDemo::startSoccerMode()
{
  ...
  RCLCPP_INFO(nh_->get_logger(), "Start Soccer Demo");
...
{
  RCLCPP_INFO(nh_->get_logger(), "Stop Soccer Demo");
  on_following_ball_ = false;
  on_tracking_ball_ = false;
...
bool SoccerDemo::isActionRunning()
{
  ...
  if (rclcpp::spin_until_future_complete(nh_, result) != rclcpp::FutureReturnCode::SUCCESS)
  {
    RCLCPP_ERROR(nh_->get_logger(), "Failed to get action status");
    return true;
  }
  else
  {
    if (result.get()->is_running == true)
    {
      return true;
    }
  }
