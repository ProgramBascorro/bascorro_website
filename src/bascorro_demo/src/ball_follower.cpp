...
#include "ball_follower.hpp"

namespace robotis_op
{

BallFollower::BallFollower()
  : nh_(rclcpp::Node::make_shared("ball_follower")),
    ...
    DEBUG_PRINT(false)
{
  current_joint_states_sub_ = nh_->create_subscription<sensor_msgs::msg::JointState>(
      "/robotis/goal_joint_states", 10, std::bind(&BallFollower::currentJointStatesCallback, this, std::placeholders::_1));

  set_walking_command_pub_ = nh_->create_publisher<std_msgs::msg::String>("/robotis/walking/command", 10);
  set_walking_param_pub_ = nh_->create_publisher<op3_walking_module_msgs::msg::WalkingParam>("/robotis/walking/set_params", 10);
  get_walking_param_client_ = nh_->create_client<op3_walking_module_msgs::srv::GetWalkingParam>("/robotis/walking/get_params");
...
{
  on_tracking_ = true;
  RCLCPP_INFO(nh_->get_logger(), "Start Ball following");
...
void BallFollower::stopFollowing()
{
  on_tracking_ = false;
  count_to_kick_ = 0;
  RCLCPP_INFO(nh_->get_logger(), "Stop Ball following");
                                ...
                                double& fb_move, double& rl_angle)
{
  ...
  RCLCPP_INFO_COND(nh_->get_logger(), DEBUG_PRINT, "distance to ball : %6.4f, fb : %6.4f, delta : %6.6f", target_distance, fb_move,
                delta_time);
  RCLCPP_INFO_COND(nh_->get_logger(), DEBUG_PRINT, "==============================================");
...
bool BallFollower::processFollowing(double x_angle, double y_angle, double ball_size)
{
  ...
  {
    RCLCPP_ERROR(nh_->get_logger(), "Failed to get current angle of head joints.");
    setWalkingCommand("stop");
  ...
  RCLCPP_INFO_COND(nh_->get_logger(), DEBUG_PRINT, "   ============== Head | Ball ==============   ");
  RCLCPP_INFO_STREAM_COND(nh_->get_logger(), DEBUG_PRINT,
  ...
  if ((distance_to_ball < distance_to_kick) && (fabs(ball_x_angle) < 25.0))
  {
    ...
    RCLCPP_INFO_STREAM_COND(nh_->get_logger(), DEBUG_PRINT, "foot to kick : " << ball_x_angle);

    RCLCPP_INFO(nh_->get_logger(), "In range [%d | %d]", count_to_kick_, ball_x_angle);
    ...
    if (count_to_kick_ > 20)
    {
      ...
      if (ball_x_angle > 0)
      {
        RCLCPP_INFO_COND(nh_->get_logger(), DEBUG_PRINT, "Ready to kick : left");  // left
        approach_ball_position_ = OnLeft;
      }
      else
      {
        RCLCPP_INFO_COND(nh_->get_logger(), DEBUG_PRINT, "Ready to kick : right");  // right
...
void BallFollower::setWalkingCommand(const std::string &command)
{
  ...
  RCLCPP_INFO_STREAM_COND(nh_->get_logger(), DEBUG_PRINT, "Send Walking command : " << command);
}
...
bool BallFollower::getWalkingParam()
{
  ...
  if (rclcpp::spin_until_future_complete(nh_, result) == rclcpp::FutureReturnCode::SUCCESS)
  {
    ...
    RCLCPP_INFO_COND(nh_->get_logger(), DEBUG_PRINT, "Get walking parameters");

    return true;
  }
  else
  {
    RCLCPP_ERROR(nh_->get_logger(), "Fail to get walking parameters.");
