    ...
    return LaunchDescription([
        Node(
            ...
            parameters=[
                ...
                ParameterFile(LaunchConfiguration('footsteps_nao'), allow_substs=True)
            ]
        ),
        ...
        DeclareLaunchArgument(
            ...
            description='Path to the NAO planning parameters YAML file'
        ),
        DeclareLaunchArgument(
            'footsteps_nao',
            default_value=[PathJoinSubstitution([FindPackageShare('footstep_planner'), 'config', 'footsteps_nao.yaml'])],
            description='Path to the NAO footsteps YAML file'
        )
    ])